...
def apply_soft_nls(ema_logit, false_target, suppression_factor=0.05):
    """
    Apply soft negative label suppression (NLS) to the EMA logit.
    Reduces the contribution of incorrect predictions based on a suppression factor.
    
    :param ema_logit: The EMA model predictions
    :param false_target: The binary matrix indicating which predictions are incorrect
    :param suppression_factor: Factor by which to suppress incorrect predictions (0 < factor < 1)
    :return: Adjusted EMA logits with suppressed incorrect predictions
    """
    ema_logit_new = ema_logit.data.cpu().numpy()
    
    # Identify the indices of incorrect predictions (where false_target == 0)
    false_label_idx1, false_label_idx2 = np.where(false_target == 0)
    
    # Suppress the incorrect logits by scaling them down
    ema_logit_new[false_label_idx1, false_label_idx2] *= suppression_factor  # Reduce the influence of false predictions

    # Convert back to a tensor
    ema_logit_new = torch.from_numpy(ema_logit_new).cuda()
    
    return ema_logit_new
...
def train(train_loader, model, ema_model, optimizer, epoch):
    ...
    for i, ((input, ema_input), (target, false_target)) in enumerate(train_loader):
        ...
        # Apply Soft NLS to the EMA logits
        ema_logit_new = apply_soft_nls(ema_logit, false_target, suppression_factor=0.5)
...
def save_checkpoint(state, is_best, dirpath, epoch):
    ...
    best_path = os.path.join(dirpath, 'best_stage2.ckpt')
